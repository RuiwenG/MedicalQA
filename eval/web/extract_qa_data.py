...
``pairs``
    ...
    footing. ``t`` (start second) is carried through when the approach records
    it, and is only ever used for the un-blinded deep link.
``videos``
        ...
        for vdir in video_dirs:
            for adir in sorted(d for d in vdir.iterdir() if d.is_dir()):
                ...
                for i, item in enumerate(items):
                    ...
                    # uses this for the deep link when blind mode is off.
...
def main() -> None:
    ...
    print(f"Pairs with a timestamp: {timestamped} / {len(pairs)} "
          f"(used only for the un-blinded deep link)")
