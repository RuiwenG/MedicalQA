...
A public, shareable version of `eval/QA_Educational_Eval_UI.ipynb`. Same eight
metrics, same 1–5 scale, same QA pairs — but an annotator only needs a link and
...
Each video links to its source on YouTube, pulled from `test_dataset.csv`
(Master) and `teepa.csv` (Teepa) by the extract script:

- a small ▶ next to every video in the picker, so you can preview before choosing;
- a **▶ Watch source video** button on each rating screen.

The rating screen shows a note that the video is optional — annotators should
rate the pair as written, especially for **Standalone Quality**.

> **Why blind sessions never deep-link to a timestamp.** Only DualAgent and
> MultiAgent record `time_start_sec` (800 of 1,587 pairs). A link that sometimes
> jumps to a moment in the video and sometimes doesn't would tell the annotator
> which system wrote the pair, defeating the blinding. So blind sessions always
> get the plain video URL, and the `&t=` deep link only appears when blind mode
> is off.
...
df.groupby("approach")[["q_fluency", "a_fluency", "q_clarity", "a_clarity",
                        "qa_alignment", "q_edu_value", "a_edu_value",
                        "standalone"]].mean().round(2)
```
...
- **Blinding.** The approach name is hidden while rating and only revealed in
  the summary at the end. It is always recorded in the stored row. Video links
  are timestamp-free in blind mode for the same reason (see above).
- **Markdown is shown raw.** Several pipelines emit `##` and `**bold**` in
  ...
  should count against Fluency and Clarity.
- **Keyboard.** `1`–`5` rates the highlighted statement and advances; `Enter`
  submits the pair. Much faster than clicking for a 200-pair batch.
